...
authorization_url = f'https://api.notion.com/v1/oauth/authorize?client_id={CLIENT_ID}&response_type=code&owner=user&redirect_uri=http%3A%2F%2Flocalhost%3A8000%2Fintegrations%2Fnotion%2Fcallback'
...
async def oauth2callback_notion(request: Request):
    logging.info("Callback function triggered")
    ...
    logging.info(f"Saved notion_credentials for {org_id}:{user_id}")
